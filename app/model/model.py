...
def fit_pipeline(X_train, y_train, pipeline, save_model=False):
    ...
    if save_model:
        joblib.dump(pipeline, 'model_cat_boost.pkl')

    return pipeline


if __name__ == '__main__':
    df = pd.read_csv("../data/data_moscow.csv", names=['okrug', 'metro', 'distance_from_center', 'route_minutes', 'total_area', 'rooms', 'price'])
    pipe = get_pipeline()
    pipe = fit_pipeline(df.drop(['price'], axis=1), df['price'], pipe, save_model=False)